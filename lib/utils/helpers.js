...
function ensureInsideProject() {
    ...
    if (!fs.existsSync(srcDir)) {
        console.log(
            chalk.red("\n❌ This command must be run inside a project created by tryappstack.")
        );
        console.log(chalk.gray("   Could not find Src/ directory.\n"));
        process.exit(1);
    }
}
...
function addExportToIndex(indexPath, exportName, fileName) {
    ...
    if (content.includes(`from './${fileName}'`)) {
        console.log(chalk.yellow(`  ⚠  ${exportName} already exists in index.js, skipping.`));
        return;
    }
...
function addImportToServerLoader(routeName) {
    ...
    if (!fs.existsSync(serverLoaderPath)) {
        console.log(chalk.red("  ❌ Src/Loaders/ServerLoader.js not found."));
        return;
    }
    ...
    if (content.includes(routeExportName)) {
        console.log(
            chalk.yellow(`  ⚠  ${routeExportName} already exists in ServerLoader.js, skipping.`)
        );
        return;
    }
