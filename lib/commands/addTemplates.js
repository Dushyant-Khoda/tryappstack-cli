...
function addTemplatesCommand(name) {
  ...
  if (fs.existsSync(filePath)) {
    console.log(chalk.yellow(`\n${PREFIX.WARN}${GENERIC.FILE_ALREADY_EXISTS(fileName + ".html")}\n`));
    process.exit(0);
  }
...
`;
  ...
  console.log(chalk.green(`${PREFIX.SUCCESS}${CREATE.CREATED_FILE("Src/Templates/" + fileName + ".html")}`));

  console.log(chalk.bold.green(`\n${CREATE.TEMPLATE_DONE(fileName + ".html")}\n`));
}
